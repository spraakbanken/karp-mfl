...
def multi_query(lexicon, fullquery, fields, query, isfilter):
    ...
    for ix, field in enumerate(fields):
        ...
        fullquery.append('and|%s.search|%s|%s' %
                         (lexconfig.get_field(lexicon, field, field),
                          operator, q))
    return fullquery
...
def authenticate(lexicon, action='read'):
    ...
    if action == 'checkopen':
        ...
        permissions = lexitems.get("lexica", {}).get(lexconfig.get_wsauth_name(lexicon), {})
        if not permissions.get(action, False):
            raise errs.MflException("Action %s not allowed in lexicon %s" %
...
def make_identifier(lexicon, baseform, pos, field='', default=False):
    ...
    raise errs.MflException("Could not come up with an identifier for %s, %s in lexicon %s" %
...
def relevant_paradigms(paradigmdict, lexicon, pos, possible_p=[]):
    ...
    try:
        ...
        raise errs.MflException("Could not read lexicon %s" % lexicon,
...
def get_current_paradigm(_id, pos, lexconf, paradigmdict):
    ...
    if not es_total(res) > 0:
        raise errs.MflException("Identifier %s not found" % _id,
    ...
    paras, numex, lms = relevant_paradigms(paradigmdict,
                                           ...
                                           pos, possible_p=[p_id])
    if not paras:
        raise errs.MflException("Paradigm %s not found" % p_id,
...
def check_identifier(_id, field, resource, mode, unique=True, fail=True):
    ...
    if not ok and fail:
        ...
        raise errs.MflException("Identifier %s %s" % (_id, text),
