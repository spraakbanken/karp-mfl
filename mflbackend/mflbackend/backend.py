...
from mflbackend import errors as e
...
from mflbackend import handleparadigms as handle
from mflbackend import helpers
from mflbackend import parsecandidates as pc
...
def inflectclass():
    ...
    paras, numex, lms = helpers.relevant_paradigms(
        paradigmdict, lexicon, pos, possible_p
    )
    # get the provided variable instances, if any
    var_inst = sorted(
        [(int(key), val) for key, val in request.args.items() if key.isdigit()]
    )
    if classname == "paradigm" and var_inst:
        ...
        if len(paras) < 1 or len(possible_p) < 1:
            raise e.MflException(
                "Cannot find paradigm %s" % classval, code="unknown_paradigm"
            )
...
def inflect():
    ...
    ans = handle.inflect_table(
        lexicon, table, paradigmdict, lemgram, pos, ppriorv, match_all=strict
    )
...
def inflectlike():
    ...
    if possible_p:
        paras, numex, lms = helpers.relevant_paradigms(
            paradigmdict, lexicon, pos, possible_p=possible_p
        )
...
def inflectcandidate():
    ...
    if helpers.es_total(res) < 1:
        raise e.MflException(
            "Could not find candidate %s" % identifier, code="unknown_candidate"
        )
    ...
    for inflection in pc.candidateparadigms(candidate):
        ...
        paras, n, l = helpers.relevant_paradigms(
            paradigmdict, lexicon, pos, possible_p=[pc.get_uuid(inflection)]
        )
        # the paradigm might be removed, then skip it
        # if not, save the table
        if paras:
            ans.append(
                helpers.make_table(lexicon, paras[0], var_inst, 0, pos, lemgram=lemgram)
            )
...
def listing():
    ...
    if s == "class":
        ...
        res = helpers.karp_query(
            "statlist",
            {"q": query, "size": size, "buckets": classname + ".bucket"},
            resource=lexconf["lexiconName"],
            mode=lexconf["lexiconMode"],
        )
        return jsonify(
            {
                "compiled_on": classname,
                "list": res["stat_table"],
    ...
    if s in ["wf", "wordform"]:
        ...
        res = helpers.karp_query(
            "statlist",
            {"q": query, "size": size, "buckets": lexconf["baseform"] + ".bucket"},
            resource=lexconf["lexiconName"],
            mode=lexconf["lexiconMode"],
        )
        return jsonify(
            {
                "compiled_on": "wordform",
                "list": res["stat_table"],
...
def _compile():
    ...
    if "," in search_f:
        ...
        helpers.multi_query(
            lexicon, query, search_f.split(","), querystr.split(","), isfilter
        )
    ...
    if compile_f == "class":
        ...
        ans = []
        for pbucket in helpers.es_get_classbucket(classname, res, lexconf)[
            int(start) :
        ]:
        ...
        show = ",".join([pp.id_field, pp.transform_field, pp.entries])
        lexicon = lexconf["paradigmlexiconName"]
        mode = lexconf["paradigmMode"]
        ans = helpers.compile_list(
            query, s_field, querystr, lexicon, show, size, start, mode, isfilter
        )
        res = []
        iclasses = []
        for hit in ans["ans"]:
...
def update_table():
    ...
    karp_id = helpers.get_es_identifier(
        identifier,
        lexconf["identifier"],
        lexconf["lexiconName"],
        lexconf["lexiconMode"],
    ...
    return jsonify(
        {
            ...
            "var_inst": dict(enumerate(v, 1)),
            "classes": classes,
            "pattern": para.jsonify(),
            "partOfSpeech": pos,
            "members": para.count,
        }
    )
...
def addcandidates():
    ...
    for table in tables:
        ...
        to_save.append(
            helpers.make_candidate(
                lexconf["candidatelexiconName"], lemgram, forms, res, pos
            )
        )
    ...
    return jsonify(
        {"saved": to_save, "candidatelexiconName": lexconf["candidatelexiconName"]}
    )
...
def recomputecandiadtes():
    ...
    for pos in postags:
        ...
        for hit in helpers.es_get_hits(res):
            ...
            # format
            to_save = helpers.make_candidate(
                lexconf["candidatelexiconName"],
                table["identifier"],
                forms,
                res,
                table["partOfSpeech"],
...
def handle_invalid_usage(error):
    try:
        ...
        if "status_code" in dir(error):
            ...
            content = json.dumps(
                {"message": "Error!!\n%s" % error.message, "code": error.code}
            )
        else:
            status = 404
            content = json.dumps(
                {"message": "Error!!\n%s" % error, "code": "unknown_error"}
            )
...
def update_model(lexicon, pos, paradigmdict, lexconf, paras=None):
    ...
    if paras is None:
        paras = read_paradigms(
            lexconf["paradigmlexiconName"], pos, lexconf["paradigmMode"]
        )
    logging.debug("memorize %s paradigms??", len(paras))
    paras, numex, lms, alphabet = mp.build(
        paras,
        lexconf["ngramorder"],
        lexconf["ngramprior"],
        lexicon=lexconf["paradigmlexiconName"],
